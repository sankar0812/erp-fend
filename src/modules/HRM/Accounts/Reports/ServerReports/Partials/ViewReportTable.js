...
export const ViewAllServerReports = () => {
  ...
  useEffect(()=>{
    const server= 'report'
    request.get(`${APIURLS.GET_SERVER_REPORT}`,{params:{server}})
  ...
  const DateSearch = (values) => {
    request
      .post(`${APIURLS.POST_SERVER_REPORTS}`, values)
      .then(function (response) {
        setDataSource(response.data);
  ...
  const columns = [
    ...
    {
        title: "ServerType Name",
        dataIndex: "server_typeName",
      },
      {
        title: "Amount",
        dataIndex: "amount",
  ...
  if (AllServerStatus === "loading") {
    ...
    const rowKey = (dataSource) => dataSource.serverListId ;
    content = (
      <CustomStandardTable
        columns={columns}
        data={dataSource}
  ...
  return (
    <Fragment>
      <CustomPageTitle Heading={"Server Report"} />
      <br />
      <Form
        form={form}
        labelCol={{
