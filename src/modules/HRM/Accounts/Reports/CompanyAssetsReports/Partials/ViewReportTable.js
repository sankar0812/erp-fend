...
export const ViewAllCompanyAssetReports = () => {
  ...
  useEffect(()=>{
    const companyassets= 'report'
    request.get(`${APIURLS.GET_COMPANYASSET_REPORT}`,{params:{companyassets}})
  ...
  //================= Date filter CompanyAssets Reports fn============
  ...
  const DateSearch = (values) => {
    request
      .post(`${APIURLS.POST_COMPANYASSET_REPORTS}`, values)
      .then(function (response) {
        setDataSource(response.data);
  ...
  if (AllAssetsStatus === "loading") {
    ...
    const rowKey = (dataSource) => dataSource.companyAssetsListId;
    content = (
      <CustomStandardTable
        columns={columns}
        data={dataSource}
  ...
  return (
    <Fragment>
      <CustomPageTitle Heading={"Company Assets Report"} />
      <br />
      <Form
        form={form}
        labelCol={{
