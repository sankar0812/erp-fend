...
export const ViewAllEmpComplaintsReports = () => {
...
//=====================
  const DateSearch = (values) => {
    if (RoleName === "Training") {
      request
      .post(`${APIURLS.POSTTRAINEE_COMPLAINTS_REPORTS}`, values)
      .then(function (response) {
        setDataSource(response.data);
      ...
      request
      .post(`${APIURLS.POSTEMPLOYEE_COMPLAINTS_REPORTS}`, values)
      .then(function (response) {
        setDataSource(response.data);
        if (response.data.length) {
          toast.success("Date Filter Search");
  ...
  return (
    <Fragment>
      {
        RoleName === "Training" ?  <CustomPageTitle Heading={"Trainee Complaints Report"} /> : <CustomPageTitle Heading={"Employee Complaints Report"} />
      }
