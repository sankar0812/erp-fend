...
import { ViewGdDetails } from "./ViewGdDetails";
...
import { getGroupDiscussion, getGroupDiscussionError, getGroupDiscussionStatus, selectAllGroupDiscussion } from "../../RecruitmentSlice";
import { CommonLoading } from "../../../../../components/CommonLoading";
import { CustomStandardTable } from "../../../../../components/Form/CustomStandardTable";
import { SvgIcons } from "../../../../../Images";
import { GDCancelForm } from "./GdCancelForm";
import { HrInterviewForm } from "../../HRInterview/Partials/HrInterviewForm";
...
export const GdTable = () => {
  ...
  const AllGroupDiscussion = useSelector(selectAllGroupDiscussion);
  const GroupDiscussionStatus = useSelector(getGroupDiscussionStatus);
  const GroupDiscussionError = useSelector(getGroupDiscussionError);

  useEffect(() => {
    setDataSourse(AllGroupDiscussion);
  }, [AllGroupDiscussion]);

  const UpdateGdDetails = (record) => {
    setTrigger(trigger + 1);
    setModalTitle("Update Group Discussion");
    setModalContent(
      <GdForm
        formname={"UpdateForm"}
        FormExternalClose={FormExternalClose}
        formReset={formReset}
        updateGdrecord={record}
  ...
  const ViewGdDetail = (record) => {
    setModalTitle("View Group Discussion");
    setModalContent(
      <ViewGdDetails
        formname={"UpdateForm"}
        FormExternalClose={FormExternalClose}
  ...
  const AddHrRound = (record) => {
    setTrigger(trigger + 1);
    setModalTitle("Add HR Round");
    setModalContent(
      <HrInterviewForm
        AddHrRound={record}
        updatetrigger={trigger}
          ...
          <Flex center={"true"} gap={"10px"}>
            ...
            {record?.approvalType === "scheduled" && (
              <TableIconHolder
                ...
                size={"22px"}
                onClick={() => {
                  UpdateGdDetails(record);
                }}
              >
                <Tooltip placement="top" title={"Edit GD"}>
            ...
            {record?.approvalType === "scheduled" && (
              ...
              >
                <Tooltip placement="top" title={"Add HR Interview"}>
                  <img src={SvgIcons.assign} width={22} alt="Add HR Interview" />
                </Tooltip>
              </TableIconHolder>
            )}
  ...
  if (GroupDiscussionStatus === "loading") {
    content = <CommonLoading />;
  } else if (GroupDiscussionStatus === "succeeded") {
  ...
  } else if (GroupDiscussionStatus === "failed") {
    content = <h2>{GroupDiscussionError}</h2>;
  }
