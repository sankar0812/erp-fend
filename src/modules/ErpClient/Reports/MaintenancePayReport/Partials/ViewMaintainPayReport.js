...
import {
  ...
  getInvoiceStatus,
  getinvoiceView,
} from "../../../../HRM/Accounts/Invoice/invoiceSlice";
import { APIURLS } from "../../../../../utils/ApiUrls/Client";

export const ViewMaintainPayReport = () => {
  ...
  const GetMaintenanceClient = () => {
    request.get(`${APIURLS.GET_INDIVIDUAL_CLIENT_MAINTENANCE}/${ClientID}`)
      .then(function (response) {
        setDataSource(response.data[0]?.maintenancePayment)
      })
  ...
  //================= Date filter Client MaintainPay Reports fn============
  ...
  const DateSearch = (values) => {
    request
      .post(`${APIURLS.POST_INDIVIDUAL_MAINTAIN_SEARCH}`, values)
      .then(function (response) {
  ...
  return (
    <Fragment>
      <CustomPageTitle Heading={"MaintenancePay Report"} />
      <br />
      <Form
        form={form}
        labelCol={{
