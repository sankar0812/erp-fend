...
import { AllTestingDocuments, getAllTestingDocuments } from '../AfterResearchDevelopment/AfterRandDSlice';
import TestingPDF from './TestingPDF';
import { selectCurrentRole } from '../../Auth/authSlice';



export const ShowingTestedDocuments = () => {
  ...
  `
    ...
    const AllTestingDocumentsDetails = useSelector(AllTestingDocuments)

    const CurrentRole = useSelector(selectCurrentRole)

    useEffect(() => {
        setViewData(AllTestingDocumentsDetails)
    }, [AllTestingDocumentsDetails,viewData])
    ...
    const ViewResearchedDoc = (doc) => {
        ...
        setModalContent(
            <TestingPDF testrec={doc} FormExternalClose={FormExternalClose} showdevtrigger={trigger} />
        );
        showModal();
    };
    ...
    const PromoteToNextStep = (doc) => {
        ...
        request.put(`${RANDDURLS.PUT_APPROVING_TESTING_TO_MANAGEMENT}${doc?.testingDocumentationId}`, newValues )
            .then(resp => {
    ...
    return (
        <div>
            ...
            {
                    viewData?.map((doc) => (
                        <Col span={24} md={8} key={doc.testingDocumentationId}>
