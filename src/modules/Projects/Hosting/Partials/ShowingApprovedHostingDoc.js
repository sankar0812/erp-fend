...
import { AllApprovedHostingDocuments, AllHostingDocuments, getAllApprovedHostingDocuments, getAllHostingDocuments } from '../../AfterResearchDevelopment/AfterRandDSlice';
...
const ShowingApprovedHostedDoc = () => {
  ...
  margin: 20px;
    ...
    useEffect(() => {
        dispatch(getAllApprovedHostingDocuments())
    }, [])

    const allHostedDoc = useSelector(AllApprovedHostingDocuments);
    ...
    return (
        <div>
            <CustomPageTitle Heading={"View Approved Hosted Documents "} />

            <CustomRow space={[24, 24]} style={{ marginTop: '30px' }}>
                {
                    viewData?.map((doc) => (
                        <Col span={24} md={8} key={doc.hostingDocumentationId}>
                            <Card style={{ background: doc.projectStatus === "approved" ? 'green' : (doc.projectStatus === "rejected" ? 'red' : '#6b5da6'), color: 'white' }}>
                                <Flex center={"true"} gap={'20px'}>
                                    ...
                                    <Tooltip title={'View PDF'}>
                                        <div style={{ color: doc.projectStatus === "rejected" ? 'yellow' : 'red', cursor: 'pointer' }} onClick={() => ViewResearchedDoc(doc)}>
                                            <FaFilePdf />
                                        </div>
                                    </Tooltip>
                                </Flex>
...
export default ShowingApprovedHostedDoc