...
import { AllApprovedRandDProjects, approvedRandDProjectError, approvedRandDProjectStatus, getApprovedRandDProjects } from '../AfterRandDSlice'
import { CustomPopconfirm } from '../../../../components/CustomPopConfirm'
import { BsClipboardCheck, BsThreeDots } from 'react-icons/bs'
import { AssigningDeptAndPLForDev } from './AssigningDeptAndPL'
import { FaPersonDotsFromLine } from 'react-icons/fa6'

export const ViewAllAfterRandDList = () => {
    ...
    const AllRandDCompletedStatus = useSelector(approvedRandDProjectStatus);
    const AllRandDCompletedError = useSelector(approvedRandDProjectError);

    useEffect(() => {
        setDataSource(AllRandDCompletedDocs)
    }, [AllRandDCompletedDocs])
    ...
    const AssignDeptandPLModal = (record) => {
        ...
        setModalTitle('Assign Dept & PL');
        setModalContent(<AssigningDeptAndPLForDev devrecord={record} FormExternalClose={FormExternalClose} devtrigger={trigger} />);
        showModal()
    }
    ...
    const handleRandDConfirm = (record) => {
        ...
        request.put(`${RANDDURLS.PUT_CHANGE_APPROVED_R_AND_D_STATUS}${record.projectAssigningId}`, newValue)
            .then(resp => {
                toast.success("Project Approved to Development")
                dispatch(getApprovedRandDProjects())
            })
            ...
            render: (record) => {
                return (
                    <Flex center={"true"} gap={'20px'}>
                        ...
                        {
                            record.projectStatus === "pending" && record?.departmentList[0]?.departmentName === null ? (
                                <Tooltip title={'Assign'}>
                                    <TableIconHolder color='red' size={"22px"}>
                                        <FaPersonDotsFromLine color='blue' onClick={() => AssignDeptandPLModal(record)} />
                                    </TableIconHolder>
                                </Tooltip>
                            ) : record.projectStatus === "pending" && record.departmentList[0]?.departmentName !== null ? (
                                <CustomPopconfirm
    ...
    if (AllRandDCompletedStatus === 'loading') {
        content = <CommonLoading />
    } else if (AllRandDCompletedStatus === 'succeeded') {
    ...
    } else if (AllRandDCompletedStatus === 'failed') {
        content = <h2>
            {AllRandDCompletedError}
        </h2>
    }
    ...
    return (
        <div>
            <CustomPageTitle Heading={"R & D Completed List"} />
            <CustomRow
                space={[24, 24]}
